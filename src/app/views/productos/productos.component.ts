...
export class ProductosComponent implements OnInit {
  ...
  actualizarProducto(): void {
    ...
    if (this.myForm.valid && this.selectedProducto) {
      ...
      this.toastType = 'error';
      this.toastMessage = 'El formulario no es válido o no hay producto seleccionado!';
      this.showToast = true;
      setTimeout(() => (this.showToast = false), 3000);
    }
  }
  ...
  inactivarProducto(producto: any): void {
    ...
    this.http.put(`http://localhost:8080/api/productos/${producto.idProducto || producto.id_producto}`, payload).subscribe(
      (response) => {
        ...
        this.toastType = 'success';
        this.toastMessage = 'Producto inactivado correctamente';
        this.showToast = true;
        this.loadproductos();
        setTimeout(() => (this.showToast = false), 3000);
      },
      (error) => {
        ...
        this.toastType = 'error';
        this.toastMessage = error.error || 'No se pudo inactivar el producto';
        this.showToast = true;
        setTimeout(() => (this.showToast = false), 3000);
      }
    );
  ...
  toggleProductoEstado(producto: any): void {
    ...
    this.http.put(`http://localhost:8080/api/productos/${producto.idProducto || producto.id_producto}`, payload).subscribe(
      ...
      (error) => {
        ...
        this.toastType = 'error';
        this.toastMessage = error.error || 'No se pudo cambiar el estado del producto';
        this.showToast = true;
        setTimeout(() => (this.showToast = false), 3000);
      }
    );
  ...
  confirmEliminar(productoId: number | undefined): void {
    ...
    if (!productoId) {
      this.toastType = 'error';
      this.toastMessage = 'ID de producto no válido para eliminación';
      this.showToast = true;
      setTimeout(() => (this.showToast = false), 3000);
      return;
    }
    ...
    this.http.delete(`http://localhost:8080/api/productos/${productoId}`).subscribe(
      (response) => {
        ...
        this.toastType = 'success';
        this.toastMessage = 'Producto eliminado permanentemente';
        this.showToast = true;
        this.loadproductos();
        setTimeout(() => (this.showToast = false), 3000);
      },
      (error) => {
        ...
        this.toastType = 'error';
        this.toastMessage = error.error || 'No se pudo eliminar el producto';
        this.showToast = true;
        setTimeout(() => (this.showToast = false), 3000);
      }
    );
